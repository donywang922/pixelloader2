...
public class MapSettingScreen extends SettingScreen {
    ...
    public void init() {
        super.init();
        mapsize = this.addRenderableWidget(Button.builder(Component.translatable("pixelLoader.setting.screen.mapsize", Setting.mapSize),
                p_onPress_1_ -> {
                    Setting.mapSize++;
                    Setting.mapSize = Setting.mapSize > 8 ? 1 : Setting.mapSize;
                    mapsize.setMessage(Component.translatable("pixelLoader.setting.screen.mapsize", Setting.mapSize));
                }).bounds(this.width / 2 - 100, height / 2 - 12, 200, 20).build());
        lt = this.addRenderableWidget(Button.builder(Component.translatable("pixelLoader.setting.screen.mapmode." + Setting.mapMode),
                p_onPress_1_ -> {
                    ...
                    lt.setMessage(Component.translatable("pixelLoader.setting.screen.mapmode." + Setting.mapMode));
                }).bounds(this.width / 2 - 100, height / 2 + 12, 200, 20).build());
    }
}
