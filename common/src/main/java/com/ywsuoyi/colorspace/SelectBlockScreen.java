    ...
    public void render(GuiGraphics poseStack, int i, int j, float f) {
        ...
        for (int idx = 0; idx < ColorSpace.selectBlocks.size(); idx++) {
            if (idx * lineHeight + lineHeight > scrollY && idx * lineHeight - lineHeight < scrollY + height) {
                ...
                poseStack.drawString(font, block.block.getName(), 220, yOffset, 0xFFFFFF);
                poseStack.drawString(font, descriptionId, 220, yOffset + 10, 0xFFFFFF);
                poseStack.drawString(font, block.active ? check : uncheck, width - 34, yOffset + 5, 0xFFFFFF);
            }
        }
    }
